...
export default function UnitPage() {
  ...
  const logExercise = useCallback(() => {
    ...
    fetch('/api/log-activity', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: 'writing', count: 1 }) }).catch(() => {});
  ...
  const nextExercise = () => {
    ...
    if (exerciseIdx + 1 >= unit.exercises.length) {
      fetch('/api/drill-progress', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ drillType: 'writing', levelId: unitId, score, total: unit.exercises.length, completed: true }) }).catch(() => {});
      setPhase('result');
    ...
    setAnimateIn(false);
    if (!isPro) fetch('/api/drill-progress', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ drillType: 'writing', levelId: unitId, score, total: freeUsed + total }) }).catch(() => {});
    setTimeout(() => {
    ...
    return (
      <div className={styles.resultScreen}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', maxWidth: 360, width: '100%' }}>
          {hasNext && (
            <Link href={`/drills/writing/${nextUnit}`} className={styles.resultLinkPrimary} style={{ background: 'linear-gradient(135deg, #ef4444, #cc0000)' }}>
              Next Unit <ArrowRight size={18} />
            </Link>
          )}
          <Link href="/ai-coach?skill=writing" className={styles.resultLinkSecondary}>
            <Sparkles size={18} /> Practice Writing with AI
          </Link>
          <Link href="/drills/writing" className={styles.resultLinkBack}>
            ← Back to Course
          </Link>
        </div>
      </div>
  ...
  if (hitPaywall) {
    ...
    return (
      <div className={styles.paywallScreen}>
        ...
        <Link href="/drills/writing" className={styles.resultLinkBack}>← Back to levels</Link>
      </div>
    );
  }
  ...
  return (
    <div className={styles.pageImmersive} style={{ opacity: animateIn ? 1 : 0, transform: animateIn ? 'translateY(0)' : 'translateY(10px)', transition: 'all 0.2s ease' }}>
      ...
      <div className={styles.progressBar}>
        <Link href="/drills/writing" className={styles.backBtn}>
          <XCircle size={22} />
        </Link>
