...
export default function UnitPage() {
  ...
  const logExercise = useCallback(() => {
    // Guest mode — increment local counter, skip API calls
    if (isGuest) {
  ...
  }, [answered, exercise, shuffledOpts, isGuest, trackExercise]);
  ...
  const nextExercise = () => {
    ...
    if (exerciseIdx + 1 >= unit.exercises.length) {
      ...
      fetch('/api/drill-progress', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ drillType: 'speaking', levelId: unitId, score, total: unit.exercises.length, completed: true }),
      }).catch(() => {});
      setPhase('result');
      if (!isPro) {
    ...
    if (!isPro) {
      fetch('/api/drill-progress', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ drillType: 'speaking', levelId: unitId, score, total: freeUsed + total }),
      }).catch(() => {});
    }
    setTimeout(() => {
  ...
  // ─── PAYWALL ───
  // Guest wall — show signup modal after 2 exercises
  // GuestWall now shown as overlay when guest tries to interact
